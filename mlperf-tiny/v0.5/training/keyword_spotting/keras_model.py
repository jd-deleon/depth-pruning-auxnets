...
def get_model(args):
  ...
  if model_name=="fc4":
  ...
  elif model_name == 'ds_cnn_flat':
    print("DS CNN model invoked")
    input_shape_flat = [model_settings['spectrogram_length']*model_settings['dct_coefficient_count']]
    ...
    # Model layers
    # Input pure conv2d
    inputs = Input(shape=input_shape_flat)
    x = tf.reshape(inputs, (-1, model_settings['spectrogram_length'],model_settings['dct_coefficient_count'],1))
    # x = Reshape((model_settings['spectrogram_length'],model_settings['dct_coefficient_count'],1), input_shape=input_shape_flat)(inputs)
